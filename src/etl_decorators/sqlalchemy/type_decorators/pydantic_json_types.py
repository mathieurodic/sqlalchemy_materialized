...
from ._pydantic_json_core import _MutablePydanticModelList, _instrument_model_cls
...
class PydanticJSON(sa.TypeDecorator):
    ...
    def __init__(self, model_cls: Type[BaseModel]):
        ...
        _instrument_model_cls(model_cls)
...
class PydanticJSONList(sa.TypeDecorator):
    ...
    def process_result_value(self, value: Any, dialect) -> Any:
        ...
        # sa.JSON usually returns list on the Python side
        if isinstance(value, list):
            out = _MutablePydanticModelList()
            out.extend([self.model_cls.model_validate(v) for v in value])
            return out
        ...
        if isinstance(value, str):
            ...
            out = _MutablePydanticModelList()
            out.extend([self.model_cls.model_validate(v) for v in parsed])
            return out

        # Fallback: treat as a single element list
        out = _MutablePydanticModelList()
        out.append(self.model_cls.model_validate(value))
        return out
